/*
 * IDeviceInstance.h - part of the ESP32-IDeviceInstance library
 *
...
public:
    ...
    /**
     * @brief Retrieve data of the specified type as float
     *
     ...
     * @note For temperature: returns scaled float (e.g., 22.5 for 22.5°C)
     */
    virtual DeviceResult<std::vector<float>> getData(DeviceDataType dataType) = 0;

    /**
     * @brief Retrieve raw integer data of the specified type
     *
     * @param dataType The type of data to retrieve
     * @return DeviceResult<std::vector<int16_t>> containing error code and raw values
     *
     * @note Thread-safe - implementations must use appropriate locking
     * @note For temperature: returns raw value, interpret with getDataScaleFactor()
     *       - LOW_RES: tenths (220 = 22.0°C)
     *       - HIGH_RES: hundredths (2200 = 22.00°C)
     * @note More efficient than getData() - avoids int→float→int conversion
     */
    virtual DeviceResult<std::vector<int16_t>> getDataRaw(DeviceDataType dataType) {
        // Default implementation: not supported (override in derived classes)
        (void)dataType;
        return DeviceResult<std::vector<int16_t>>(DeviceError::NOT_SUPPORTED);
    }

    /**
     * @brief Get scale divider for raw data interpretation
     *
     * @param dataType The type of data
     * @return Divider to convert raw to actual value (e.g., 10 or 100 for temperature)
     *
     * @note For temperature: divide raw by divider for whole part, modulo for fraction
     *       - LOW_RES: 10 (220 / 10 = 22, 220 % 10 = 0 → 22.0°C)
     *       - HIGH_RES: 100 (2200 / 100 = 22, 2200 % 100 = 0 → 22.00°C)
     * @note Default returns 1 (no scaling)
     */
    virtual int16_t getDataScaleDivider(DeviceDataType dataType) const {
        (void)dataType;
        return 1;
    }

    /**
     * @brief Get scale divider for a specific channel
     *
     * @param dataType The type of data
     * @param channel The channel index (0-based)
     * @return Divider to convert raw to actual value
     *
     * @note Some devices have per-channel scaling based on sensor type
     *       Example: MB8ART PT1000 supports HIGH_RES (÷100), PT100 does not (always ÷10)
     * @note Default delegates to getDataScaleDivider(dataType)
     */
    virtual int16_t getDataScaleDivider(DeviceDataType dataType, uint8_t channel) const {
        (void)channel;
        return getDataScaleDivider(dataType);
    }
